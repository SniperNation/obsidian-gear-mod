...
public class TpVoidCommand {
	...
	public static void registerCommands(RegisterCommandsEvent event) {
		event.getDispatcher()
				.register(Commands.literal("tpvoid").requires(s -> s.hasPermission(4))
						.then(Commands.argument("arguments", StringArgumentType.greedyString()).executes(TpVoidCommand::execute))
						.executes(TpVoidCommand::execute));
	}
	...
	private static int execute(CommandContext<CommandSourceStack> ctx) {
		...
		TpVoidCommandExecutedProcedure.execute(entity);
		return 0;
	}
}
