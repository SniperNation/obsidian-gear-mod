...
package net.mcreator.obsidiangear.world.teleporter;
...
public class TheVoidTeleporter implements ITeleporter {
	...
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("the_void_portal",
				Sets.newHashSet(ImmutableSet.copyOf(ObsidianGearModBlocks.THE_VOID_PORTAL.getStateDefinition().getPossibleStates())), 0, 1)
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? ObsidianGearModBlocks.VOID_PORTAL_FRAME.defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, ObsidianGearModBlocks.VOID_PORTAL_FRAME.defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = ObsidianGearModBlocks.THE_VOID_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getPortalRepositioner(entity, blockpos1).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = TheVoidPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return TheVoidPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
